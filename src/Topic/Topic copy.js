...
import BarGraph from '../Bar/RaceBar.js'
...
export default class Topic extends Component {
    ...
    render(){
        ...
        var scatterY = this.options["variable"][2];
        var scatterX = this.options["scatterVar"];
        ...
        // grouping
        var alldata = require('../data/'+topicIndex.toString() +
                            '_'+graphTypes[graphIndex]+'.json');

        var currentVar = this.options["variable"][graphIndex];

        if(!alldata.hasOwnProperty(currentVar)){
            currentVar = Object.keys(alldata)[0];
            this.options["variable"][graphIndex] = currentVar;
        }

        this.groupingdata = alldata[currentVar]["group"];
        ...
        // normalize
        this.normalizedata = alldata[currentVar]["normalizer"];
        ...
        if(graphIndex===0 || this.topicIndex!=topicIndex){
            this.timelineData = require('../data/'+topicIndex.toString()+'_Line.json');
        ...
        // --------------------------- Scatter --------------------------- //
        if(graphIndex===2 || this.topicIndex!=topicIndex){
            this.scatterData = require('../data/'+topicIndex.toString()+'_Scatter.json');
            this.scatterHeader = [];
            this.menuVariableItems[2] = [];
            for(key in this.scatterData){
                this.scatterHeader.push(key);
                this.menuVariableItems[2].push(
                    <MenuItem key={key} value={key}>{key}</MenuItem>
                );
            }

            if(!this.scatterData.hasOwnProperty(scatterY)){
                this.options["variable"][2] = this.scatterHeader[0];
                scatterY = this.scatterHeader[0];
            }

            if(!this.scatterData.hasOwnProperty(scatterX)){
                this.options["scatterVar"] = this.scatterHeader[1];
                scatterX = this.scatterHeader[1];
            }
            ...
            //         var t_entry = this.scatterData[lineVariable]["data"][
            //             this.groupingdata["_index"][selectedIndexes]];
            //         var filteredEntry = {data:[],id:t_entry.id}
        ...
        return (
            <div style={{
                ...
                <Row style={{width:"100%", margin:"0", padding:"0"}}>
                    ...
                    {/* // --------------------------- Scatter --------------------------- // */}
                    <div style={{width:"80%", display:display[2]}}>
                        {<ScatterGraph 
                                    titleX={this.scatterData[scatterX]["title"]}
                                    titleY={this.scatterData[scatterY]["title"]}
                                    dataGenerator={this.scatterData[scatterY]["data"]}/>}
                    </div>
                    {/* // --------------------------- Options --------------------------- // */}
